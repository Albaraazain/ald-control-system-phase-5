#!/usr/bin/env python3
"""
Terminal 2 Launcher - Recipe Service

Command-line launcher for Terminal 2 with the same interface as main.py
but focused exclusively on recipe command processing and execution.

Usage:
  python terminal2_launcher.py --plc real --ip 192.168.1.50
  python terminal2_launcher.py --demo  # simulation mode
  python terminal2_launcher.py --doctor  # connection test only
"""
import sys
import os
import argparse
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Terminal 2 - Recipe Service")
    ...
    # Terminal 2 specific options
    parser.add_argument("--poll-interval", type=float, default=2.0,
                        help="Recipe command polling interval in seconds (default: 2.0)")
    parser.add_argument("--command-timeout", type=float, default=300.0,
                        help="Recipe execution timeout in seconds (default: 300.0)")
...
def apply_env_overrides(args):
    ...
    # Terminal 2 specific overrides
    if args.poll_interval:
        os.environ["TERMINAL2_POLL_INTERVAL"] = str(args.poll_interval)
    if args.command_timeout:
        os.environ["TERMINAL2_COMMAND_TIMEOUT"] = str(args.command_timeout)
    if args.machine_id:
...
def main_cli():
    ...
    try:
        ...
        _logger.info("Terminal 2 - Recipe Service starting")
        _logger.info(
    ...
    # Import and run Terminal 2 service
